...
export const updateCart = async (
  ...
  try {
    ...
    if (params.addLines && params.addLines.length > 0) {
      const addResponse = await storefrontClient.query<{ data: { cartLinesAdd: { cart: CartDetails } } }>({
        data: {
    ...
    if (params.removeLineIds && params.removeLineIds.length > 0) {
      const removeResponse = await storefrontClient.query<{ data: { cartLinesRemove: { cart: CartDetails } } }>({
        data: {
    ...
    if (params.updateLines && params.updateLines.length > 0) {
      const updateResponse = await storefrontClient.query<{ data: { cartLinesUpdate: { cart: CartDetails } } }>({
        data: {
